...
IDENTITY
    ...
    either level. A directory's own mode and mtime appear in no
    listing (a listing records children). Nothing outside a scanned
...
PARTIAL SCANS
    ...
    contributes no bytes to any ancestor's size. Sockets, fifos, and
    devices are reported and omitted. The same tree under the same
...
DESCRIPTION
    ...
    IDs are equal. For a snapshot-ID target, the plan's first line is
    the same ID that --force prints as line 1.
...
NOTES
    ...
    and independent inodes, wherever the link partners lived. Symlink
    entries carry no ID and always materialize from their recorded
    name and target. The root directory's own mode and mtime are
    recorded nowhere: <dir> itself keeps platform-default metadata,
    and verification is unaffected. A null-ID target entry — any
    entry whose recorded ID is null, the mark of something unreadable
    at scan time, file or directory (symlink entries record no ID by
    rule and are never omissions) — cannot be materialized, and
    restore keeps what it finds: it never creates, modifies, or
    removes anything at that entry's name; whatever <dir> holds there
    is left exactly as found, and an absent name stays absent. The
    rest is restored, each omission is listed on stderr, and restore
    exits 2 with line 2 printed. Re-running cannot close an omission:
    the target itself lacks the knowledge. restore never deletes
    <dir> itself: undoing a restore into a previously absent
    directory leaves an empty directory. Concurrent restores of the
    same directory are not coordinated: the store stays safe; the
    tree's final state is undefined. Content read from stores other
    than the write store is outside the write store's locks; a read
    that fails mid-run exits 3 and line 1's undo stands.
...
EXIT STATUS
    ...
    2 reconciled with omissions: the resulting tree differs from the
      target only at target entries whose recorded ID is null — file
      or directory, kept as found (listed on stderr); line 2 is the
      verified as-built ID
