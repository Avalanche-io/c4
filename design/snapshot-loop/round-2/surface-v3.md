...
DESCRIPTION
    ...
    listing (root included, at both full and content detail), and one
    entry in the store journal per path. -s writes only inside the
    store directory: it creates no .c4m file, nothing in the scanned
    tree, nothing in your working directory. stdout is one line per
    path — the snapshot ID, in argument order — each printed only
    after everything that path stored is on stable media. Paths are
    snapshotted in argument order and claimed independently: an
    earlier snapshot stands even if a later path fails, and all paths
    are attempted. With a file argument, -s stores the file's bytes
    and journals them (a file's snapshot ID is the ID of its bytes;
    restore targets are directory descriptions).
...
IDENTITY
    ...
    the total bytes of the files it contains; a symlink's size is the
    byte length of its target text. A symlink entry records the link's
    name and target text, with mode, time, and ID null in every
    listing form; a symlink names no store object. A directory's own
    mode and mtime appear in no listing (a listing records children).
    Nothing outside a scanned tree affects that tree's ID, and a
    directory's ID is the same whether it is scanned directly or as a
    subdirectory of a larger scan.
...
DURABILITY
    ...
    its journal entry — survives power loss, even one microsecond
    after printing. If it did not print, nothing was claimed: the
    scanned tree was never written to, the store's prior contents and
    journal are intact, and re-running the same command continues
    where it left off (objects already present are skipped). One
    in-between case exists: if power failed after the journal entry
    but before the print, the snapshot is complete and durable and
    `c4 log` lists it — whatever the journal shows after a crash is
    real. These guarantees follow from the write ordering alone, so
...
SYNOPSIS
    ...
    target:  a C4 ID resolved from the store (a snapshot ID, a content
             ...
             and never take /path.
    dir:     the directory to reconcile; created if absent. If it
...
DESCRIPTION
    ...
    patch format, computed at the target's knowledge level: the first
    line is <dir>'s current ID at that level, then one line per
    differing entry, then the target's ID. <dir> already matches
    exactly when — equivalently — the plan has no entry lines and its
    two boundary IDs are equal. For a snapshot-ID target, the plan's
    first line is the same ID that --force prints as line 1.
    ...
    With --force:
      ...
      2. If <dir> contains anything a snapshot cannot fully record
         (unreadable files or directories, sockets, fifos, devices),
         restore refuses, lists the offenders on stderr, and exits 1
         without touching <dir>. Symlinks — broken or not — are fully
         recordable: a link is its name and target.
      3. <dir>'s current state is snapshotted into the store —
      ...
      4. <dir> is reconciled, then verified (see VERIFICATION), then
         line 2 prints.
...
VERIFICATION
    ...
    is recomputed from the resulting tree. Content-ID target: the
    content-form listing is recomputed — names, sizes, symlink names
    and targets, and bytes are verified; modes and times are platform-
    default metadata, meaning whatever file creation under your umask
    and clock produced — not recorded by the target, not claimed, and
    not verified. Symlink entries recompute as name, target text, and
...
UNDO
    ...
    command, and recorded permanently in the store journal. `c4 log`
    is the reflog: a lost terminal does not lose the undo handle.
...
DURABILITY
    ...
    restore line 1 to go back.

NOTES
    A description records names, sizes, modes, mtimes, symlink names
    and targets, hard links within the tree, sequences, and content
    IDs. It does not record ownership, extended attributes, ACLs, or
    special files; restore neither preserves nor restores those. A
    hard-link partner outside the snapshot root restores as an
    independent file with identical bytes. Symlink entries carry no ID
    and always materialize from their recorded name and target. The
    root directory's own mode and mtime are recorded nowhere: <dir>
    itself keeps platform-default metadata, and verification is
    unaffected. Target file entries with null IDs cannot be
    materialized: the rest is restored, the omissions are listed on
    stderr, and restore exits 2. restore never deletes <dir> itself:
    undoing a restore into a previously absent directory leaves an
    empty directory. Concurrent restores of the same directory are not
    coordinated: the store stays safe; the tree's final state is
    undefined. Content read from stores other than the write store is
    outside the write store's locks; a read that fails mid-run exits 3
    and line 1's undo stands.
...
EXIT STATUS
    ...
    1 refused before any modification: missing content, uncapturable
      entries, unresolvable or unparseable target, or <dir> exists and
      is not a directory — nothing modified
    2 reconciled with omissions: the resulting tree differs from the
      target only at file entries whose target ID is null (listed on
      stderr); line 2 is the verified as-built ID
    3 modification began but the resulting tree differs from the
      target in any other way (an operation failed, concurrent
      mutation, a store read failed): line 1 printed and its undo
      stands; line 2 prints if recomputation completed; re-run to
      finish, or restore line 1 to go back
...
DESCRIPTION
    ...
    stdout, whatever they are — the store has no object types. A
    listing is c4m text, so catting a listing shows the directory's
    one-level children. Forms that must interpret the object — /path
...
PATHS
    ...
    Final       The last component may name a directory (yields its
                ...
                its ID list; members are fetched by the IDs in that
...
DESCRIPTION
    ...
    when that side is a description.
...
DESCRIPTION
    ...
    log lists those chains' sections the same way.
...
OUTPUT
    ...
    `c4 log | tail -1` is the latest ingest. Output is byte-stable: a
    line, once printed, is printed identically by every future run.
    The first field is the index — the N that c4 split accepts — and
    the last field is the ID. The journal file itself is the record;
    log output is a view of it.
...
DESCRIPTION
    ...
    stopped (the swap is a plain file rename and takes no lock), then
    run c4 gc --force.
