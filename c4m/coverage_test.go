...
func TestSequenceExpansionEdgeCases(t *testing.T) {
	t.Run("expand sequence without idList", func(t *testing.T) {
		seqEntry := &Entry{Name: "file.[001-003].txt", Mode: 0644, Size: 100, IsSequence: true}

		expanded, err := expandSequenceEntry(seqEntry, nil)
		if err != nil {
...
func TestSequenceExpansionDirectCoverage(t *testing.T) {
	t.Run("expand regular entry fails", func(t *testing.T) {
		entry := &Entry{Name: "file.txt", Mode: 0644, Size: 100, Timestamp: time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC)}
		_, err := expandSequenceEntry(entry, nil)
		if err == nil {
			t.Error("expected error for non-sequence entry")
		}
	})
	t.Run("expand sequence entry", func(t *testing.T) {
		entry := &Entry{Name: "file.[001-002].txt", Mode: 0644, Size: 100, Timestamp: time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC), IsSequence: true}
		results, err := expandSequenceEntry(entry, nil)
		if err != nil {
			t.Fatalf("expandSequenceEntry: %v", err)
		}
		if len(results) != 2 {
...
func TestEncodingMoreEdgeCases(t *testing.T) {
	t.Run("encode produces no directives", func(t *testing.T) {
		manifest := NewManifest()
		manifest.AddEntry(&Entry{Name: "file.txt", Mode: 0644, Size: 100})
		var buf bytes.Buffer
		if err := NewEncoder(&buf).Encode(manifest); err != nil {
...
// TestSequenceExpansionCoverage tests expandSequenceEntry
func TestSequenceExpansionCoverage(t *testing.T) {
	t.Run("expand with nil idList", func(t *testing.T) {
		entry := &Entry{Name: "file.[001-002].txt", Mode: 0644, Size: 100}
		results, err := expandSequenceEntry(entry, nil)
		if err != nil {
			t.Logf("expandSequenceEntry: %v", err)
		}
		_ = results
	})
	t.Run("expand non-sequence name", func(t *testing.T) {
		_, err := expandSequenceEntry(&Entry{Name: "file.txt", Mode: 0644, Size: 100}, nil)
		if err == nil {
			t.Error("expected error for non-sequence name")
		}
	})
}
