...
// The reference pages below are the CLI's contract, written per
// design/snapshot-loop/round-1/surface-v7.md and amended by
// round-2/draft-v8.md (re-scan trust; journal times carry the scan
// start). `c4 --help` prints the top page; each verb's --help prints
// its own page.
...
const idHelp = `C4-ID(1) - describe files and directories as c4m text; snapshot with -s
SYNOPSIS
    c4 id [-q] [-s] [-m s|c|m|f] [-e] [-S] [-c GUIDE.c4m] [--verify]
...
ARGUMENTS
    ...
    resolve to final state). Every other file argument is bytes, and files inside scanned trees
    are ALWAYS bytes, whatever their name. A description computes at its own knowledge level, so
    its -q ID is its own canonical ID (-m projects downward only; ingesting a description stores
    its listings, not the content they refer to). An argument whose first slash-separated
    component is syntactically a C4 ID - 90 chars, c4 prefix, all base58 - is a store address in
    every verb (tested before the .c4m rule; prefix ./ to force a filesystem path), accepted by
    c4 cat, restore targets, and diff sides; id never reads the store - it exits 1 pointing at:
    c4 cat -r <ID> | c4 id -q -
...
EXCLUSIONS
    ...
    what was scanned, never what was omitted. Right after a snapshot, c4 diff <SNAP> <dir>
    lists exactly what was left out; once the tree drifts, exclusions and ordinary edits are
    indistinguishable there. WARNING: restoring a snapshot taken with exclusions removes what
    was excluded from the live tree (the pre-image holds it; see c4-restore(1) NOTES).
    Exclusions never change exit status. restore --force's pre-image is always a complete
    default scan: exclusions never narrow the safety net.
DURABILITY
    ...
    before its journal entry; the entry is stable before the ID prints. Enforcement per platform:
      macOS/Linux  staged objects get cheap per-file fsync; one device-cache barrier makes all
                   staged bytes stable, then renames claim their hash names (Linux: containing
                   directories fsynced); the journal entry is appended and fsynced; a second
                   barrier precedes the print. Measured (APFS, M-series): 20,312 files / 105 MB
                   ingests with store in ~4.3 s; materialize ~4.4 s; no-op re-apply ~0.3 s.
    ...
    A repeat snapshot without -c re-reads and re-hashes every byte - no timestamp is trusted -
    and skips only the store writes of objects already present. The skip is safe two ways: an
    object can appear at its hash name only after its bytes reached stable media (the barrier
    precedes every rename), and every later read rehashes, so a wrong object at a right name
    cannot pass silently.
RE-SCAN TRUST
    -c GUIDE.c4m makes the re-read itself skippable, by trust stated out loud. A file reuses
    the guide's recorded ID - without being read - exactly when its path is in the guide, its
    size and mtime (at second precision) equal the guide's entry, and its mtime is strictly
    older than the guide's scan start. The scan start comes from the store journal: the claim
    whose ID equals the guide's root ID (the guide file's own mtime stands in when no claim
    matches - never earlier than the scan it recorded). A file whose mtime is not strictly
    older than that instant could have been written during or after that scan within the
    filesystem's timestamp granularity, so it is re-hashed regardless of apparent equality -
    git's racy-index resolution restated, with no tunable window. Everything else - changed,
    racy, or new - is re-hashed; the stderr summary counts the split: reuse: R reused,
    H rehashed.

    The accepted, documented risk: a same-size byte change under a deliberately restored older
    mtime is INVISIBLE to a -c re-scan - the guide's ID is reused for stale bytes. Bytes
    already ingested are immune (every store read re-verifies). --verify is the audit: it
    ignores every reuse rule, re-hashes everything named, and with a guide present reports each
    file whose bytes changed under unchanged size+mtime on stderr:
        verify: content changed under unchanged metadata: <path>
PARTIAL SCANS
...
EXAMPLES
    ...
    c4 id -s -c prior.c4m src/                              # fast re-snapshot (trusted mtimes)
    c4 id --verify -c prior.c4m src/ >/dev/null             # audit: full re-hash + report
`

const restoreHelp = `C4-RESTORE(1) - make a directory match a description, undo-safely
SYNOPSIS
    ...
    target  a store-resolved C4 ID (snapshot, content, or any stored listing ID), optionally
            ...
            extract files with c4 cat); or a c4m file (chains resolve). Must resolve to a
            listing; a directory is never a target (mirror: EXAMPLES).
...
DESCRIPTION
    ...
    target's knowledge level: line 1 is <dir>'s current ID at that level, one line per differing
    entry, then the target ID. Matching = no entry lines = equal boundary IDs.

    With --force: (1) every ID the target names must resolve, else listed on stderr - exit 1,
    <dir> untouched; (2) <dir>'s pre-image is snapshotted - a complete default scan, nothing
    excluded, nothing folded - durable and journaled before the first destructive operation,
    printed as stdout line 1; (3) reconcile - entries the target does not record are removed,
    recorded entries materialize; (4) verify by recomputation, print line 2.
VERIFICATION
    Line 2 is observed from the filesystem in this run: the as-built tree is rescanned at the
    target's knowledge level and must recompute to the target's own root ID. Content-ID targets
    verify names, sizes, symlink targets, bytes; modes/times are unclaimed platform defaults.
...
UNDO
    ...
    depth. Handles are IDs, never refs: stdout line 1 (scripts take stdout), and permanent in
    the journal - c4 log is the reflog; a lost terminal loses nothing. No c4 verb deletes store
    objects, so every undo handle keeps working for as long as the store directory exists
    (c4-log(1) RETENTION).
OUTPUT
    Dry run: the plan. --force: at most two lines on stdout, each a bare ID, nothing else -
      ...
      line 2  the verified, recomputed ID of <dir> as restore left it, at the target's level;
              printed whenever recomputation completed
    Absent or empty <dir>: line 1 is the empty-description constant (c4-id(1)); restoring that
    ID empties a directory. Counts and narration are stderr. Capture stdout whole, then split;
    a lost line is never lost state - line 1 is journaled, and the as-built ID is recomputable:
    c4 id -q -m f <dir>.
NOTES
    ...
    removed at that name; each is declared on stderr and owns exit 2. Permission, space, and
    I/O failures are curable and exit 3. Concurrent restores of one directory are
    uncoordinated: the store stays safe; the tree's outcome is undefined. restore never deletes
    <dir> itself: undoing a restore into a previously absent <dir> leaves it empty.
EXIT STATUS
    0 plan emitted, or reconciled and verified equal to the target
    1 refused before any modification - nothing modified
    2 reconciled with declared omissions, each listed on stderr; line 2 printed and true
    3 curable failure after line 1 (permission, space, transient I/O, concurrent mutation, a
      failed store read): line 1's undo stands; line 2 prints if recomputation completed;
      cure, then re-run.
EXAMPLES
...
const catHelp = `C4-CAT(1) - retrieve store objects by ID or ID/path; display c4m files
SYNOPSIS
...
DESCRIPTION
    ...
    exactly what it folds. Equal states emit nothing. c4 diff <SNAP> <dir> right after a
    snapshot shows everything the snapshot left out.
`

const patchHelp = `C4-PATCH(1) - resolve patch chains to c4m text
SYNOPSIS
    c4 patch [-n N] [-e] <chain.c4m>...
DESCRIPTION
    patch resolves chains - c4m text in, c4m text out (-n N picks a section; multiple files
    concatenate into one chain) - and never touches directories; a directory argument exits 1
    with:
